...
- Group accounts: a company's own standalone turnover may differ from its
  ...
  for your use case.
- **Scanned/image-only accounts filings are invisible to both bulk and API
  mode, in every month, regardless of company size or audit status.** Both
  modes only ever see the iXBRL-tagged rendering of a filing — if a company
  submitted its accounts as a plain scanned PDF (e.g. a signed audit copy
  uploaded as-is rather than generated by accounting software), Companies
  House never creates an iXBRL version at all, so it's absent from every
  monthly Accounts Data Product archive and from the Document API's iXBRL
  endpoint alike. This isn't a timing issue or a bug in this tool — confirmed
  case: **THE FLOORBRITE GROUP LIMITED** (company no. 01219526), audited by
  Forvis Mazars LLP, turnover £29,815,970 for the year ended 31 July 2025
  (well inside the default band), filed 08 Jan 2026 as a 31-page scanned PDF
  with no text layer — checked and absent from the December 2025, January
  2026, and February 2026 archives alike. There is no automated fix for this
  short of adding OCR support; treat it as a class of company that needs a
  manual check via Companies House's website if you suspect the discovered
  company list is missing someone you'd expect to see.
